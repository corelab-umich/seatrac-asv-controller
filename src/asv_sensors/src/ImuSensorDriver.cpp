...
ImuSensorDriver::ImuSensorDriver() : Node("imu_sensor_driver") {
    ...
    imu_msg_pub_ = this->create_publisher<sensor_msgs::msg::Imu>(
        "/imu", rclcpp::SensorDataQoS());
}

void ImuSensorDriver::callback(
    const std_msgs::msg::ByteMultiArray::SharedPtr raw_data) {
    // check if the packet received is actually an IMU message packet
    if (raw_data->data.size() != asv::messages::ImuMessage::buffer_size ||
        raw_data->data[SINK_ID_BYTE_INDEX] !=
            asv::messages::ImuMessage::sink_id) {
        std::cout << "size: " << raw_data->data.size()
                  << ", sink id: " << (int)raw_data->data[SINK_ID_BYTE_INDEX]
                  << std::endl;
        return;
    }

    auto asv_imu_msg = asv::messages::ImuMessage::decode(raw_data->data.data(),
                                                         raw_data->data.size());
    ...
    quat.setRPY(to_radians(asv_imu_msg.roll), to_radians(asv_imu_msg.pitch),
                to_radians(asv_imu_msg.heading));
    quat.normalize();
