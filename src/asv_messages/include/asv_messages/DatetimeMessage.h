...
struct DatetimeMessage
{
   ...
   * `buffer_size`
   * @return populated `DatetimeMessage` struct
   */
  static DatetimeMessage decode(const unsigned char* buf, size_t buf_len);

  /**
   * @brief Encodes this `DatetimeMessage` struct into a byte buffer according to the
   * SeaTrac Datetime udp packet specification. No error checking is performed
   */
